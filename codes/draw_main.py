...
class CWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.word = QLineEdit()
        self.word.setText(word.randFromDB())

        self.guess_button = QPushButton("Guess!")

        hbox.addWidget(self.word)
        ...
        # 그룹박스6(computer의 대답)
        gb = QGroupBox('컴퓨터의 대답')
        self.cpAnswer = QLineEdit()

        left.addWidget(gb)

        hbox = QHBoxLayout()
        hbox.addWidget(self.cpAnswer)
        gb.setLayout(hbox)
